...
void parse_cmd(char command,char *line) {
  ...
  switch(command) {
    ...
    case MOVEFORWARD_TIME:
      ...
      sleep(atoi(arg_value)); 
      //send_forward_time(atoi(arg_src),atoi(arg_dst),5);
      break;
...
#ifdef __DEBUG__    
    printf("get_data after ret = %d \n", ret);
#endif
    value = get_data(client_message);
#ifdef __DEBUG__
    printf("%x %x %x %x\n",value[0],value[1],value[2],value[3]);
#endif
    return_value = (value[0] << 24 ) | (value[1] << 16) | (value[2] << 8) | (value[3]);
    free(value);
    return (return_value);
